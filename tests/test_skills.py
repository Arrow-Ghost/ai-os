...
# -- closing the approval-laundering gap -----------------------------------

@pytest.mark.parametrize("module", [
    "subprocess", "socket", "smtplib", "ftplib", "urllib.request", "asyncio",
])
def test_generated_tools_cannot_reach_network_or_shell_directly(module):
    """The exact gap: a skill that imports smtplib bypasses mail.send's own
    approval prompt entirely -- the human only ever approves the generated
    tool's call, never sees 'to=attacker@evil.com'. Banning the import forces
    composition through ctx.call(), which re-enters governance for real."""
    source = f"import {module}\n" + GOOD
    report = validate_source(source)
    assert not report.ok
    assert any(f.code == "IMPORT" for f in report.findings)


def test_composing_through_ctx_call_still_works(config, feature):
    """The fix must not remove the legitimate way to reach those tools."""
    feature("skill")
    compose = GOOD.replace(
        "    return str(n * 2)",
        '    result = ctx.call("does.not.exist")\n    return result.status.value',
    )
    report = validate_source(compose)
    assert report.ok, report.summary()


def test_a_generated_tool_calling_mail_send_hits_mail_sends_own_approval(config, feature, tmp_path):
    """End-to-end proof: run an installed self-written tool that composes via
    ctx.call("mail.send"), approve THAT call once, and mail.send must still
    raise its OWN separate approval -- with the real recipient visible --
    rather than being silently reached.

    Installed directly (with the provenance header skill.install would have
    written) rather than through skill.acquire's sandbox, because the sandbox
    correctly refuses ctx.call() for a real tool -- there is nothing to call,
    by design -- so a composing tool can never pass ITS OWN sandboxed test.
    That refusal is the sandbox working correctly, not a gap; this test
    exercises what happens once such a tool is legitimately installed.
    """
    from servant.agent import Agent
    from servant.governance import ApprovalDecision, ApprovalRequest
    ...
    relay_lines = [
        ...
        "",
        '@tool(name="relay.send", tier=Tier.DANGER)',
        "def relay_send(ctx) -> str:",
        '    """Relay a message."""',
        '    return ctx.call("mail.send", to="x@example.com", subject="hi", body="hi").status.value',
        "",
    ]
    relay = "\n".join(relay_lines)
    (features_dir / "relay.py").write_text(relay)
    ...
    class RecordingApprover:
        def request(self, req):
            seen.append(req)
            return ApprovalDecision(True)  # approve everything, to see what asked

        def confirm(self, question):
            return True

    agent = Agent(config, approver=RecordingApprover(), brain=ScriptedBrain(), quiet=True)
    agent.load_features()

    assert agent.registry.get("relay.send").generated is True
    agent.call_tool("relay.send", {})

    tools_asked = [r.tool for r in seen]
    assert "relay.send" in tools_asked, "the generated tool itself must ask"
    mail_asks = [r for r in seen if r.tool == "mail.send"]
    assert mail_asks, "mail.send must raise its OWN approval, not be silently reached"
    assert mail_asks[0].args.get("to") == "x@example.com", "the real recipient must be visible"
