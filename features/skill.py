...
3. SANDBOX. Draft code is executed only in a limited subprocess with a stub
   context -- no real memory, no network calls through the brain, no
   credentials, CPU and memory capped.
...
def skill_acquire(
    ...
    for attempt in range(1, attempts + 1):
        ...
        candidate.write_text(_provenance(capability, ctx.run_id) + source, encoding="utf-8")

        # No project_root: the sandbox finds the servant package itself, which
        # is not necessarily the agent's working root.
        sandbox = run_tool(candidate, tool_name, args, timeout=timeout)
        if sandbox.ok:
...
def _provenance(capability: str, run_id: str) -> str:
    return (
        f'"""Generated by servant.\n\n'
        f"Capability: {capability}\n"
        f"Run: {run_id}\n\n"
        f"This file was written by a language model and passed static validation\n"
        f"and a sandboxed test run. A human still reviewed it before install.\n"
        f'"""\n'
    )
...
def skill_install(ctx, name: str) -> str:
    ...
    return (
        ...
        f"It loads on the next run. It is tiered '{_tier_floor(ctx).value}', so it will "
        f"still ask before acting. Lower that in config/policy.yaml only if you trust it."
    )
